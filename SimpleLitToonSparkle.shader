Shader "Okano/Simple Lit Toon Sparkle" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			v2f vert (appdata v) {
				...
				o.uv2 = TRANSFORM_TEX(v.uv, _EmissionMap);
...
float3x3 tbn = float3x3(i.tangentDir, i.bitangentDir, i.normalDir);
				...
				// indirect light stuffs
				float3 reflectionMap = DecodeHDR(UNITY_SAMPLE_TEXCUBE_LOD(unity_SpecCube0, normalize((_WorldSpaceCameraPos - i.posWorld.xyz)), 7), unity_SpecCube0_HDR)* 0.02;
				...
				fixed4 ramp = tex2D(_ToonRamp, float2(lightShading.x,lightShading.x));

				// real samplers
				fixed4 baseColor = tex2D(_MainTex, i.uv0);
				fixed4 emission = tex2D(_EmissionMap, i.uv2);
				
				// now the final colors,
				fixed3 final = baseColor.rgb;

				// mix lighting
				final *= lerp(directLighting, indirectLighting, ramp);

				// mix emission
				final = saturate(final.rgb + EffectProcMain(i.uv0, float4(emission.rgb, 1)));
				

				// final.rgb = normalMap.xyz;
